export const mockMonuments = [
  {
    id: "1",
    name: "Colosseum",
    location: "Rome, Italy",
    period: "72-80 AD",
    description: "The Colosseum is an oval amphitheatre in the centre of the city of Rome, Italy. It is the largest ancient amphitheatre ever built, and is still the largest standing amphitheatre in the world today.",
    significance: "Symbol of Imperial Rome and one of the New Seven Wonders of the World. It could hold an estimated 50,000 to 80,000 spectators and was used for gladiatorial contests and public spectacles.",
    facts: [
      "Built of travertine limestone, tuff, and brick-faced concrete",
      "Could be flooded for naval battle reenactments",
      "Had a complex underground structure called the hypogeum",
      "Free admission for all Roman citizens"
    ],
    image: "https://images.unsplash.com/photo-1552832230-c0197dd311b5?w=800"
  },
  {
    id: "2",
    name: "Taj Mahal",
    location: "Agra, India",
    period: "1632-1653",
    description: "The Taj Mahal is an ivory-white marble mausoleum on the south bank of the Yamuna river. It was commissioned by the Mughal emperor Shah Jahan to house the tomb of his favorite wife, Mumtaz Mahal.",
    significance: "A UNESCO World Heritage Site and widely recognized as 'the jewel of Muslim art in India'. It is regarded by many as the best example of Mughal architecture and a symbol of India's rich history.",
    facts: [
      "Took 22 years and 20,000 workers to complete",
      "Changes color depending on the time of day",
      "The four minarets lean slightly outward to protect the tomb in case of earthquake",
      "Cost approximately 32 million rupees to build (around $1 billion today)"
    ],
    image: "https://images.unsplash.com/photo-1564507592333-c60657eea523?w=800"
  },
  {
    id: "3",
    name: "Great Wall of China",
    location: "Northern China",
    period: "7th century BC - 1644 AD",
    description: "The Great Wall of China is a series of fortifications made of stone, brick, tamped earth, wood, and other materials. It was built along the historical northern borders of China to protect against invasions.",
    significance: "The world's longest wall and biggest ancient architecture. It has been a UNESCO World Heritage Site since 1987 and represents one of the most impressive architectural feats in history.",
    facts: [
      "Total length of all sections ever built is 21,196 km",
      "Cannot be seen from space with the naked eye",
      "Over 1 million workers died during its construction",
      "Contains watchtowers, barracks, and garrison stations"
    ],
    image: "https://images.unsplash.com/photo-1508804185872-d7badad00f7d?w=800"
  },
  {
    id: "4",
    name: "Machu Picchu",
    location: "Cusco, Peru",
    period: "1450-1572",
    description: "Machu Picchu is a 15th-century Inca citadel located in the Eastern Cordillera of southern Peru. Often referred to as the 'Lost City of the Incas', it is the most familiar icon of Inca civilization.",
    significance: "A UNESCO World Heritage Site and one of the New Seven Wonders of the World. It demonstrates the Inca's mastery of architecture and engineering in harmony with the natural landscape.",
    facts: [
      "Built without wheels, iron tools, or mortar",
      "Abandoned during the Spanish Conquest and unknown to the outside world until 1911",
      "Contains about 200 structures",
      "Located at 2,430 meters above sea level"
    ],
    image: "https://images.unsplash.com/photo-1587595431973-160d0d94add1?w=800"
  },
  {
    id: "5",
    name: "Eiffel Tower",
    location: "Paris, France",
    period: "1887-1889",
    description: "The Eiffel Tower is a wrought-iron lattice tower on the Champ de Mars in Paris. Named after engineer Gustave Eiffel, it was built as the entrance arch for the 1889 World's Fair.",
    significance: "Global cultural icon of France and one of the most recognizable structures in the world. It is the most-visited paid monument in the world with millions of visitors annually.",
    facts: [
      "Stands 330 meters tall (1,083 feet)",
      "Weighs 10,100 tons",
      "Painted every seven years using 60 tons of paint",
      "Was the world's tallest structure until 1930"
    ],
    image: "https://images.unsplash.com/photo-1511739001486-6bfe10ce785f?w=800"
  },
  {
    id: "6",
    name: "Pyramids of Giza",
    location: "Giza, Egypt",
    period: "2580-2510 BC",
    description: "The Giza pyramid complex includes the Great Pyramid of Giza, the Pyramid of Khafre, and the Pyramid of Menkaure. They are the oldest and only remaining wonder of the ancient world.",
    significance: "Testament to ancient Egyptian civilization and engineering prowess. The Great Pyramid was the tallest man-made structure for over 3,800 years.",
    facts: [
      "The Great Pyramid contains 2.3 million stone blocks",
      "Originally covered in smooth white limestone casing",
      "Aligned to true north with only 3/60th degree of error",
      "Contains three known chambers inside"
    ],
    image: "https://images.unsplash.com/photo-1539650116574-8efeb43e2750?w=800"
  }
];