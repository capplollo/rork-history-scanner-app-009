// Simple API service that works directly in Rork environment
...
export async function callOpenAI(messages: OpenAIMessage[]): Promise<string> {
  try {
    console.log('🔑 API SERVICE - Using secure API key');
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error('Invalid OpenAI API key. Please check the API key configuration.');
